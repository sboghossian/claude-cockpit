...
- **a11y** — the sidebar inherits VSCode's own theme, so it is as accessible as the editor the user already chose. `prefers-reduced-motion` honored on every CSS animation + Talk's rAF loop. Section rail, tab row and header carry full ARIA decoration.
...
**Tabs, by section** — every tab is always available; pick a section, then a tab:

| Section | Tabs |
| --- | --- |
| **Now** | Overview · Now · Watchtower · Office · **Approve** · **Replay** |
...
`cmd/ctrl+1..9` jumps to a tab within the active section.
...
- **Header strip**: brand mark + global search + Help (?). One line, always visible.
- **Status bar**: workspace name, total tokens this session, files touched
- **Sidebar webview** (Activity Bar) — tabbed:
  - **Overview** (default): the at-a-glance dashboard — greeting, inbox, stats grid, quick actions, tokens, cost, routines.
...
2. **Open the Cockpit:** click the Cockpit icon in the Activity Bar (left rail). On first launch you'll see a **Welcome** tab with a system-check checklist and "Next steps" buttons.
...
- **No configuration.** As of v1.4 there is no widget picker, tab show/hide, layout preset, or theme selector. Every tab is always available, and the theme follows VSCode.
